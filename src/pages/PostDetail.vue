    ...
    <div v-if="post" class="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-6">
      ...
      <aside class="lg:col-span-4 space-y-6">
        ...
        >
          ...
          <div class="space-y-4">
            <RouterLink
              v-for="p in recentPosts.filter((p) => p.slug !== post.slug).slice(0, 4)"
              :key="p.slug"
              :to="`/posts/${p.slug}`"
              class="group block"
            >
              <h4 class="text-sm font-bold text-slate-800 dark:text-slate-200 group-hover:text-accent dark:group-hover:text-accent-light transition-colors line-clamp-1">
                {{ p.title }}
              </h4>
              <p class="text-[10px] text-slate-400 dark:text-slate-500 mt-1 font-bold uppercase">{{ p.date }}</p>
            </RouterLink>
          </div>
        </div>
      </aside>
...
import { getPostBySlug, renderMarkdown, getRecentPosts } from '@/utils/markdown'
