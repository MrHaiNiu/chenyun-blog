  ...
  >
    ...
    <nav class="hidden md:flex items-center justify-between px-6 py-2.5">
      ...
      <RouterLink to="/" class="flex items-center gap-2 group flex-shrink-0">
        <img
          ...
          class="w-9 h-9 rounded-full object-cover border-2 border-white/30 shadow-lg group-hover:rotate-[360deg] transition-transform duration-700"
        />
        <span class="text-base font-black text-white tracking-wider drop-shadow-lg">
          {{ siteConfig.navTitle }}
          <span class="text-accent-light">{{ siteConfig.navSuffix }}</span>
          {{ siteConfig.navAfter }}
        </span>
      </RouterLink>
    ...
    <nav class="flex md:hidden items-center justify-between px-4 py-2.5">
      <RouterLink to="/" class="flex items-center gap-2 group">
        ...
        />
        <span class="text-sm font-black text-white tracking-wider drop-shadow-lg">
          {{ siteConfig.navTitle }}
          <span class="text-accent-light">{{ siteConfig.navSuffix }}</span>
          {{ siteConfig.navAfter }}
        </span>
      ...
      <div class="flex items-center gap-1">
        ...
        <button @click="toggleMobileMenu" class="w-8 h-8 rounded-lg flex items-center justify-center text-white/70 hover:text-white hover:bg-white/10 transition-all duration-200">
          <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <Transition name="mobile-menu">
      <div v-if="isMobileMenuOpen" class="md:hidden bg-slate-900/90 backdrop-blur-md border-t border-white/10">
        <RouterLink
          v-for="link in navLinks"
          :key="link.path"
          :to="link.path"
          @click="isMobileMenuOpen = false"
          class="block px-6 py-2.5 text-sm font-bold text-white/80 hover:text-white hover:bg-white/5 transition-colors"
        >
          {{ link.label }}
        </RouterLink>
      </div>
